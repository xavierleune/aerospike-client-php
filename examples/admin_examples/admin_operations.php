...
$start = __LINE__;
$status = $db->grantPrivileges("example_role", array(array("code"=>Aerospike::READ_WRITE_UDF)));
sleep(1);
if ($status == Aerospike::OK) {
    echo success();
} else {
...
$start = __LINE__;
$status = $db->revokePrivileges("example_role", array(array("code"=>Aerospike::READ_WRITE_UDF)));
sleep(1);
if ($status == Aerospike::OK) {
    echo success();
} else {
