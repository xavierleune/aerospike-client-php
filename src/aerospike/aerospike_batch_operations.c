	...
	if (!(as_val*)(key_p->valuep)) {
		if (!null_flag) {
			if (0 != add_assoc_zval(outer_container_p, (char *)key_p->digest.value, inner_container_p)) {
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_SERVER, "Unable to get key of a record");
		...
		switch (((as_val*)(key_p->valuep))->type) {
			...
			case AS_INTEGER:
				if (!null_flag) {
					if (FAILURE == add_index_zval(outer_container_p, key_p->value.integer.value, inner_container_p)) {
						PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_SERVER, "Unable to get key of a record");
						DEBUG_PHP_EXT_DEBUG("Unable to get key of a record");
					}
				} else {
...
batch_exists_cb(const as_batch_read *results, uint32_t n, void *udata)
{
	...
	for (i = 0; i < n; i++) {
		...
		if (results[i].result == AEROSPIKE_OK) {
			...
			if (0 != AEROSPIKE_ADD_ASSOC_LONG(record_metadata_p,
				PHP_AS_RECORD_DEFINE_FOR_GENERATION, results[i].record.gen)) {
					...
					PHP_EXT_SET_AS_ERR(udata_ptr->error_p, AEROSPIKE_ERR_SERVER, "Unable to get generation of a record");
					goto cleanup;
			}
			if (0 != AEROSPIKE_ADD_ASSOC_LONG(record_metadata_p,
				PHP_AS_RECORD_DEFINE_FOR_TTL, results[i].record.ttl)) {
					DEBUG_PHP_EXT_DEBUG("Unable to get ttl of a record");
					PHP_EXT_SET_AS_ERR(udata_ptr->error_p, AEROSPIKE_ERR_SERVER, "Unable to get ttl of a record");
				goto cleanup;
			}
			null_flag = false;
		...
		populate_result_for_get_exists_many((as_key *)results[i].key, udata_ptr->udata_p,
			AEROSPIKE_ZVAL_ARG(record_metadata_p), udata_ptr->error_p, null_flag TSRMLS_CC);
	...
	zval *keys_p, zval *metadata_p, zval *options_p TSRMLS_DC)
 {
	...
	for (i = 0; i < list->size; i++) {
		...
		populate_result_for_get_exists_many_new((as_key *)(&(record_batch->key)),
			metadata_callback.udata_p, AEROSPIKE_ZVAL_ARG(record_metadata_p),
		...
		if (AEROSPIKE_OK != aerospike_get_key_meta_bins_of_record_new(NULL,
			...
			AEROSPIKE_ZVAL_ARG(record_metadata_p), NULL, null_flag, false TSRMLS_CC)) {
		...
		if (!null_flag) {
			if (0 != add_assoc_zval(AEROSPIKE_ZVAL_ARG(record_metadata_p), PHP_AS_RECORD_DEFINE_FOR_BINS,
				AEROSPIKE_ZVAL_ARG(get_record_p))) {
					DEBUG_PHP_EXT_DEBUG("Unable to get the record");
					PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to get the record");
					goto cleanup;
			}
		} else {
			if (0 != add_assoc_null(AEROSPIKE_ZVAL_ARG(record_metadata_p), PHP_AS_RECORD_DEFINE_FOR_BINS)) {
				DEBUG_PHP_EXT_DEBUG("Unable to get the record");
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to get the record");
				goto cleanup;
			}
...
batch_get_cb(const as_batch_read *results, uint32_t n, void *udata)
{
	...
	for (i = 0; i < n; i++) {
		...
		foreach_record_callback_udata.udata_p = AEROSPIKE_ZVAL_ARG(get_record_p);
		...
		populate_result_for_get_exists_many((as_key *)results[i].key, udata_ptr->udata_p,
			AEROSPIKE_ZVAL_ARG(record_p), udata_ptr->error_p, false TSRMLS_CC);
		if (AEROSPIKE_OK != udata_ptr->error_p->code) {
			DEBUG_PHP_EXT_DEBUG("%s", udata_ptr->error_p->message);
			goto cleanup;
		}
		...
		if (AEROSPIKE_OK != aerospike_get_key_meta_bins_of_record_new(NULL,
			...
			AEROSPIKE_ZVAL_ARG(record_p), NULL, null_flag, false TSRMLS_CC)) {
		...
		if (!null_flag) {
			if (0 != add_assoc_zval(AEROSPIKE_ZVAL_ARG(record_p), PHP_AS_RECORD_DEFINE_FOR_BINS,
				AEROSPIKE_ZVAL_ARG(get_record_p))) {
			...
			if (0 != add_assoc_null(AEROSPIKE_ZVAL_ARG(record_p), PHP_AS_RECORD_DEFINE_FOR_BINS)) {
				PHP_EXT_SET_AS_ERR(udata_ptr->error_p, AEROSPIKE_ERR_RECORD_NOT_FOUND,
					"Unable to get a record");
				DEBUG_PHP_EXT_DEBUG("Unable to get a record");
				goto cleanup;
	...
	zval *keys_p, zval *records_p, zval *filter_bins_p, zval *options_p TSRMLS_DC)
{
	...
	for (i = 0; i < list->size; i++ ) {
		...
		populate_result_for_get_exists_many_new((as_key *)(&(record_batch->key)),
			batch_get_callback_udata.udata_p, AEROSPIKE_ZVAL_ARG(record_p_local),
		...
		if (AEROSPIKE_OK != aerospike_get_key_meta_bins_of_record_new(NULL,
			(as_record *)&(record_batch->record), (as_key *)&(record_batch->key),
			AEROSPIKE_ZVAL_ARG(record_p_local), NULL, null_flag, false TSRMLS_CC)) {
				DEBUG_PHP_EXT_DEBUG("Unable to get metadata of the record");
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to get metadata of the record");
				goto cleanup;
		}
		...
		if (!null_flag) {
			if (0 != add_assoc_zval(AEROSPIKE_ZVAL_ARG(record_p_local), PHP_AS_RECORD_DEFINE_FOR_BINS,
				AEROSPIKE_ZVAL_ARG(get_record_p))) {
					DEBUG_PHP_EXT_DEBUG("Unable to get the record");
					PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to get the record");
					goto cleanup;
			...
			if (0 != add_assoc_null(AEROSPIKE_ZVAL_ARG(record_p_local), PHP_AS_RECORD_DEFINE_FOR_BINS)) {
				DEBUG_PHP_EXT_DEBUG("Unable to get the record");
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to get the record");
				goto cleanup;
			}
