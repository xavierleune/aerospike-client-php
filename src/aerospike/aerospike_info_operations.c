	...
	zval* response_str_p, zval* host, zval* options_p TSRMLS_DC)
{
	...
	if (host) {
		...
		#endif
			{
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to find addr");
			...
			#endif
			{
				PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Unable to find port");
				DEBUG_PHP_EXT_DEBUG("Unable to find port");
				goto exit;
			}
		if ((AEROSPIKE_Z_TYPE_P(host_name) != IS_STRING) || (AEROSPIKE_Z_TYPE_P(port) != IS_LONG))
		{
			PHP_EXT_SET_AS_ERR(error_p, AEROSPIKE_ERR_PARAM, "Host parameters are not correct");
...
{
	as_policy_info              info_policy;
	DECLARE_ZVAL(response_services_p);
	DECLARE_ZVAL(response_service_p);
#if PHP_VERSION_ID < 70000
	...
	if (AEROSPIKE_OK != aerospike_info_specific_host(as_object_p, error_p,
