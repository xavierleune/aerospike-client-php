...
typedef struct data_list_map {
    ...
    as_list     alloc_list[MAX_AS_LIST_SIZE];
    u_int32_t   current_map_idx_u32;
    as_map  alloc_map[MAX_AS_MAP_SIZE];
} as_data_list_map_struct;

extern as_status
aerospike_switch_key_type(as_key* as_key_p,
                          int8_t* namespace_p, int8_t* set_p, zval** record_pp)
{
    ...
    switch(key_type) {
        case IS_LONG:
            ...
            break;
        case IS_STRING:
            as_key_init_str(&key, namespace, set, (int8_t *) Z_STRVAL_PP(record_key));
            break;
        default:
            status = AEROSPIKE_ERR;
            break;
...
static as_status
aerospike_transform_fill_data(HashTable* ht_p, as_record* record_p, zval** record_pp)
{
    as_status         status = AEROSPIKE_OK;

    if ((!ht_p) || (!record_p) || (!record_pp)) {
        status = AEROSPIKE_ERR;
        goto exit;
    }
...
static as_status
aerospike_transform_key_data(HashTable* ht_p)
{
    as_status        status = AEROSPIKE_OK;
    HashPosition     hashPosition_p = NULL;
    zval**           record_pp = NULL;
